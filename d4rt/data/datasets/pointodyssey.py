...
class PointOdysseyDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if max_start <= 0:
            ...
            frame_indices = np.arange(start_idx, start_idx + self.num_frames * self.frame_stride, self.frame_stride)
        ...
        for fi in frame_indices:
            if fi < len(rgb_files):
                ...
                frames.append(frames[-1] if frames else np.zeros((*self.resolution, 3), dtype=np.uint8))
        ...
        # Sample query points (prefer visible ones)
        first_frame_vis = visibs[frame_indices[0]]
        visible_indices = np.where(first_frame_vis)[0]

        if len(visible_indices) >= self.num_queries:
            query_indices = np.random.choice(visible_indices, self.num_queries, replace=False)
        else:
            # Mix visible and random
            n_visible = min(len(visible_indices), self.num_queries // 2)
            n_random = self.num_queries - n_visible
            vis_sample = np.random.choice(visible_indices, n_visible, replace=False) if n_visible > 0 else np.array([], dtype=int)
            rand_sample = np.random.choice(num_points, n_random, replace=True)
            query_indices = np.concatenate([vis_sample, rand_sample])
        ...
        queries = {
            "uv": torch.from_numpy(query_uv).float(),  # (Q, 2)
            "t_src": torch.from_numpy(t_src).long(),  # (Q,)
            "t_tgt": torch.from_numpy(t_tgt).long(),  # (Q,)
            "t_cam": torch.from_numpy(t_cam).long(),  # (Q,)
        }
        ...
        targets = {
            ...
            "visibility": torch.from_numpy(target_vis).float(),  # (Q,)
        }
