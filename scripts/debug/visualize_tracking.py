...
def run_inference(model, sample, device):
    ...
    with torch.no_grad():
        output = model(video, queries, cameras)
...
def visualize_tracks(sample, output, save_path: str, num_frames: int = 8, num_points: int = 16):
    ...
    gt_vis = sample['targets']['visibility'].numpy()  # (Q,)
    query_uv = sample['queries']['uv'].numpy()  # (Q, 2)
    t_src = sample['queries']['t_src'].numpy()  # (Q,)
    t_tgt = sample['queries']['t_tgt'].numpy()  # (Q,)

    # Get predictions
    pred_uv = output['uv'].squeeze(0).cpu().numpy()  # (Q, 2)
    pred_conf = output['confidence'].squeeze(0).cpu().numpy()  # (Q,)
    ...
    for col, fi in enumerate(frame_indices):
        ...
        for i, idx in enumerate(indices):
            ...
            if t_tgt[idx] == fi:
                ...
                axes[0, col].scatter(gt_x, gt_y, c=[colors[i]], marker=marker, s=100, edgecolors='white', linewidths=1)

                # Prediction (bottom)
                pred_x = pred_uv[idx, 0] * (W - 1)
                pred_y = pred_uv[idx, 1] * (H - 1)
                alpha = pred_conf[idx]
                axes[1, col].scatter(pred_x, pred_y, c=[colors[i]], marker='o', s=100, alpha=alpha, edgecolors='white', linewidths=1)

            # Also show source point
            if t_src[idx] == fi:
                src_x = query_uv[idx, 0] * (W - 1)
                src_y = query_uv[idx, 1] * (H - 1)
                axes[0, col].scatter(src_x, src_y, c=[colors[i]], marker='s', s=80, edgecolors='black', linewidths=2)
                axes[1, col].scatter(src_x, src_y, c=[colors[i]], marker='s', s=80, edgecolors='black', linewidths=2)
    ...
    legend_elements = [
        ...
        plt.Line2D([0], [0], marker='x', color='gray', linestyle='None', markersize=10, label='Target occluded'),
    ]
...
def visualize_error_distribution(sample, output, save_path: str):
    ...
    gt_vis = sample['targets']['visibility'].numpy()
    pred_uv = output['uv'].squeeze(0).cpu().numpy()
    pred_conf = output['confidence'].squeeze(0).cpu().numpy()
...
def main():
    ...
    for i in range(args.num_samples):
        ...
        print(f"\nSample {i+1}/{args.num_samples} (scene: {sample['metadata']['scene']})")
